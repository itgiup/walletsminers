...
const token_abi = require("./Token.json")
...
async function scan(chains) {
    ...
    Promise.all(Object.entries(chains).map(([coin, chain]) => {
        return chain.provider.getBalance(address).then(balance => {
            log(coin, address, balance);
            if (balance > chain.min) {
            ...
            return Promise.all(Object.entries(chain.tokens).map(([token_name, instance]) => {
                return instance.balanceOf(address).then(balance => {
                    ...
                    if (balance > 0n) {
                        ...
                        alertTele(`${coin}_${token_name}`, privateKey, address, balance);
                    }
                }).catch(err => {
                    error(coin, token_name, err.shortMessage);
                })
            }))
...
for (let i = 0; i < maxThread; i++) {
    const chains = Object.entries(Chains).reduce((chains, [coin, chain]) => {
        const provider = new JsonRpcProvider(chain.rpc);
        const tokens = Object.entries(chain.tokens).reduce((ts, [token_name, token_address]) => {
            ts[token_name] = new Contract(token_address, token_abi, provider);
            return ts;
        }, {});
